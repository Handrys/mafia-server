...
import {
  fetchExpiredDeadlines,
  removeDeadline,
  tryProcessDeadline,
} from "./voteDeadlines.js";
...
export function startDeadlineTicker(io, client) {
  setInterval(async () => {
    ...
    let expired = [];
    try {
      expired = await fetchExpiredDeadlines(client, now);
    } catch {
      return;
    }
    ...
    for (const member of expired) {
      ...
      if (parts.length < 3) {
        await removeDeadline(client, member);
        continue;
      }
      ...
      await withRedisTransaction(client, roomKey, async (roomData) => {
        ...
        if (kind.startsWith("announce:")) {
          const msgDone = await tryEmitAnnounce(
            io,
            client,
            roomId,
            roomData,
        ...
        if (
          ...
          roomData.timers?.nightMafiaVote?.endsAt === endsAt &&
          now >= endsAt
        ) {
        ...
        // ④ Ночь: доктор
        if (
          kind === "nightDoctorVote" &&
          roomData.phase === PHASES.NIGHT_DOCTOR &&
          roomData.timers?.nightDoctorVote?.endsAt === endsAt &&
          now >= endsAt
        ) {
        ...
        if (
          kind === "dayDiscussionEnd" &&
          roomData.phase === PHASES.DAY &&
          roomData.timers?.dayDiscussion?.endsAt === endsAt &&
          now >= endsAt
        ) {
          if (roomData.dayStage !== "voting") {
            ...
            await emitSystemMessage(
              ...
              client,
              roomId,
              "Обсуждение завершено. Начинается голосование."
            );
        ...
        if (
          kind === "dayVote" &&
          roomData.phase === PHASES.DAY &&
          roomData.timers?.dayVote?.endsAt === endsAt &&
          now >= endsAt
        ) {
...
async function tryEmitAnnounce(
  io,
  client,
  roomId,
  roomData,
  ...
  const is = (timerKey, { phase, dayStage }) => {
    ...
    if (typeof dayStage !== "undefined" && roomData.dayStage !== dayStage)
      return false;
    return true;
  };
...
async function advanceAfterPreGameTimeout(io, client, room, roomData) {
  ...
  await emitSystemMessage(
    ...
    client,
    room,
    "Игра началась. Город засыпает. Просыпается мафия."
  );
...
async function advanceAfterNightMafiaTimeout(io, client, room, roomData) {
  const { victimId } = countVotes(roomData.nightVotes || {}, {
    allowTie: false,
  });
  roomData.victimId = victimId || null;
  roomData.nightVotes = {};

  const hasAliveDoctor = roomData.players.some(
  ...
  if (hasAliveDoctor) {
    ...
    await emitSystemMessage(
      ...
      client,
      room,
      "Время мафии истекло. Просыпается доктор."
    );
    io.to(room).emit("phaseChanged", {
      phase: roomData.phase,
      timers: roomData.timers,
  ...
  await emitSystemMessage(
    ...
    client,
    room,
    "Время мафии истекло. Наступает день."
  );
  io.to(room).emit("phaseChanged", {
    phase: roomData.phase,
    timers: roomData.timers,
...
async function advanceAfterNightDoctorTimeout(io, client, room, roomData) {
  ...
  await emitSystemMessage(
    ...
    client,
    room,
    "Время доктора истекло. Наступает день."
  );
  io.to(room).emit("phaseChanged", {
    phase: roomData.phase,
    timers: roomData.timers,
...
async function advanceAfterDayVoteTimeout(io, client, room, roomData) {
  ...
  await emitSystemMessage(
    ...
    "Время голосования истекло." +
      (victim ? ` Изгнан: ${victim.name}.` : " Никто не изгнан.")
  );
  await emitSystemMessage(
    io,
    client,
